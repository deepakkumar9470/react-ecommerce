  ...
  return (
    <div className='p-5 transition-all duration-100 hover:scale-105 cursor-pointer'>
        <div className='bg-[#DDE6ED]  max-w-[250px] rounded-xl py-4 p-4 relative'>
          <img className='w-full h-[180px] mix-blend-multiply   rounded-md object-contain' src={product.image} alt="singleimg" />
        ...
        <div className='flex flex-col gap-2 pt-2 mb-2'>
            <p className='text-base text-softTxt font-medium'>{(product.description.substring(0,30)+ "...")}</p>
              <Rating rating={product.rating.rate} count={product.rating.count}/>

           <div className='flex items-cener gap-2'>
           <Link to={`product/${product.id}`}>
             <button 
                className='w-[100px] text-txtColor  text-sm py-2 mt-2 px-3 border-2 border-softTxt  rounded-full bg-bgLight  border-0 font-bold hover:bg-orange hover:text-white  transition duration-75'>
                View
             </button>
             </Link>
             ...
             <Plus 
               onClick={()=>addToCartHandler(product)} 
             className="md:hidden mt-4 w-8 h-8 text-base bg-[#F94C10] rounded-full text-txtColor font-bolde" size={20}/>
           </div>
